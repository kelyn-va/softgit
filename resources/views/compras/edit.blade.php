...
@section('titleContent')
    <h1 class="text-center my-4 fw-bold text-dark">Actualizar compra</h1>
@endsection
                    ...
                    <form action="{{ route('compras.update',$compra->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="precioCompra" name="precioCompra" 
                                class="form-control @error('precioCompra') is-invalid @enderror" value="{{ $compra->precioCompra }}">
                            @error('precioCompra')
                        ...
                        <div class="mb-3">
                            <label for="precioVenta" class="form-label fw-semibold">precio de la Venta</label>
                            <input type="decimal" id="precioVenta" name="precioVenta" 
                                class="form-control @error('precioVenta') is-invalid @enderror" value="{{ $compra->precioVenta }}">
                            @error('precioVenta')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="decimal" id="Total" name="Total" 
                                class="form-control @error('Total') is-invalid @enderror" value="{{ $compra->Total }}">
                            @error('Total')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                       ...
                       <option value="Efectivo">Efectivo</option>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="Cantidad" name="Cantidad" 
                                class="form-control @error('Cantidad') is-invalid @enderror" value="{{ $compra->Cantidad }}">
                            @error('Cantidad')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="text-end">
                            <button type="submit" class="crearBtn">
                                ➕ <i class="bi bi-person-plus"></i> Actualizar Empleado
                            </button>
                        </div>
                    </form>
